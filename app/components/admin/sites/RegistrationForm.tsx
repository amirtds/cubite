...
function RegistrationForm({ site }: { site: Site }) {
  ...
  const handleUpdateForm = async () => {
    const result = await fetch(`/api/site/${site.domainName}/registrationForm`, {
      method: "PUT",
      body: JSON.stringify({
        siteId: site.id,
        registrationForm: { title: formTitle, description: formDescription, buttonText: formButtonText },
      }),
    });
  };
