...
const Course = ({ params: { id } }: Props) => {
  ...
  const handleCourseDescription = (e) => {
    course ? (course.description = e.target.value) : "";
    setCourse(course);
  };

  const handleSubjects = (options) => {
    course ? (course.subjects = options) : "";
    setCourse(course);
  };

  const handleTopics = (options) => {
    course ? (course.topics = options) : "";
    setCourse(course);
  };

  const handleInstructors = (options) => {
    course ? (course.instructors = options) : "";
    setCourse(course);
  };

  const handleSites = (options) => {
    course ? (course.sites = options) : "";
    setCourse(course);
  };

  const handleLevel = (e) => {
    course ? (course.level = e.target.value) : "";
    setCourse(course);
  };

  const handlePrice = (e) => {
    course ? (course.price = parseFloat(e.target.value)) : "";
    setCourse(course);
  };
  ...
  const handleEndDate = (endDate) => {
    course ? (course.endDate = endDate) : "";
    setCourse(course);
  };

  const handleCertificateTitle = (e) => {
    course ? (course.certificateTitle = e.target.value) : "";
    setCourse(course);
  };

  const handleCertificateDescription = (e) => {
    course ? (course.certificateDescription = e.target.value) : "";
    setCourse(course);
  };

  const handleCertificateBackground = (certificateBackgroundSrc) => {
    course ? (course.certificateBackground = certificateBackgroundSrc) : "";
    setCourse(course);
  };
  ...
  const handleEnrollmentExpiration = (expirationDate) => {
            ...
            <div className="my-10 grid grid-cols-1 gap-x-6 gap-y-8  sm:grid-cols-6">
              ...
              <div className="sm:col-span-2">
                <label className="form-control w-full max-w-xs">
                  ...
                  <div className="label">
                    <span className="label-text-alt">Change Course Level</span>
                  </div>
                </label>
              </div>
              <div className="sm:col-span-2">
          ...
          >
            <div className="my-10 grid grid-cols-1 gap-x-6 gap-y-8  sm:grid-cols-6">
              ...
              <div className="sm:col-span-2">
                <label className="form-control w-full max-w-xs">
                  <div className="label">
                    <span className="label-text">Certificate Description</span>
                  </div>
                  <input
                    type="text"
                  ...
                  <div className="text-center">
                    ...
                    <div className="mt-4 flex text-sm leading-6">
                      <CldUploadWidget
                        ...
                        onSuccess={(results, options) => {
                          handleCertificateBackground(results.info?.public_id);
                        }}
                      >
                        {({ open }) => {
                          return (
              ...
              <dialog id="add_enrollment" className="modal">
                <div className="modal-box">
                  ...
                  {addEnrollmentStatus === 201 ? (
                    ...
                    <p className="py-4">
                      Please fill the following form to add a new enrollment to
                      your course.
                    </p>
                  ) : (
                  ...
                  <div className="">
                    <form method="dialog" onSubmit={handleAddEnrollmentSubmit}>
                      <DateTimeInput
