...
const CourseAuthoring = ({ params: { id } }: Props) => {
  ...
  const [postContentVersions, setPostContentVersions] = useState([]);
  ...
  useEffect(() => {
    ...
    const getPostContentVersions = async () => {
      const response = await fetch(`/api/post-content-versions/${id}`);
      const result = await response.json();
      if (result.status === 200) {
        setPostContentVersions(result.changeLog);
      }
    };
